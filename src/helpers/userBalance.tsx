...
export async function balance(address:String) {
   ...
   let temp=await connection.getBalance(_address,'confirmed');
   console.log("COnnection value:--",connection);
   console.log("Balance --",temp);
...
export async function publicAddress(address:String) {
    let _address=new PublicKey(address);
...
export async function airDrop(address:String) {
    const connection = await connectTo();
    let _address=await publicAddress(address);
    const airdropSignature = await connection.requestAirdrop(_address,9999);
